...
[AddComponentMenu("CustomEvents/MouseEventManager", 0)]
public class MouseEventManager : UniqueGameSettingBaseInScene<MouseEventManager>
{
    [ShowInInspector]
    ...
    [LabelText("优先检测哪个维度的物体")] public ObjectDimensions dimensionsDetectPriority = ObjectDimensions.TWO_D;
    ...
    [BoxGroup("Only For Debugging"), ReadOnly, ShowInInspector]
    ...
    protected override void Awake()
    {
        ...
        {
            Note.note.Warning($"MouseEventController总数超出1，" +
    ...
    private static MouseEventTrigger Detect3DTrigger()
    {
        ...
        if (Physics.Raycast(ray, out var hit3D))
        {
            ...
            if (detectResult.objectDim == ObjectDimensions.THREE_D)
            {
                ...
                if (detectResult.bindWindow == default)
                {
                    return detectResult;
                }

                if (detectResult.bindWindow.isOn)
                {
                    return detectResult;
                }
    ...
    private static MouseEventTrigger Detect2DTrigger()
    {
        ...
        foreach (Vector2 direction in hitDirections)
        {
            ...
            RaycastHit2D newHit =
                Physics2D.Raycast(new Vector2(ray.origin.x, ray.origin.y), direction, detectDistance2D);
            if (newHit.collider)
            {
                Vector3 colliderPos = bindCamera.WorldToScreenPoint(newHit.collider.gameObject.transform.position);
                colliderPos.z = 0;
        ...
        if (distance >= 0)
        {
            ...
            if (detectResult.objectDim == ObjectDimensions.TWO_D)
            {
                ...
                if (detectResult.bindWindow == default(WindowEvent))
                {
                    return detectResult;
                }
